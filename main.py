...
plt.savefig("/inputData_AAL.png", dpi=fig.dpi)
...
TOTAL_SIZE = mid_prices.size
#split into test and training data
train_data = mid_prices[:TRAINING_SIZE]
test_data = mid_prices[TRAINING_SIZE:]
...
#smoothing the data since stocks are ragged
smoothing_window_size = 1000

for di in range(0, TRAINING_SIZE,smoothing_window_size):
...
plt.savefig("/standardAverage_AAL.png", dpi=fig.dpi)
...
plt.ylabel('Mid Price')
plt.legend(fontsize=18)
plt.savefig("/runningAverage_AAL.png", dpi=fig.dpi)
...
#accumulative losses and predictions
train_mse_ot = []
test_mse_ot = []
...
test_points_seq = np.arange(TRAINING_SIZE,TOTAL_SIZE,50).tolist()
...
plt.xlabel('Date',fontsize=18)
plt.ylabel('Mid Price',fontsize=18)
plt.xlim(TRAINING_SIZE,TOTAL_SIZE)
...
plt.ylabel('Mid Price',fontsize=18)
plt.xlim(TRAINING_SIZE,TOTAL_SIZE)
plt.savefig("/ltsmResults_AAL.png", dpi=fig.dpi)
